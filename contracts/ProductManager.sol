...
contract ProductManager is CropManager {
    ...
    function deleteCertificate(address ETHAddress) public {
        for (uint i=0;i<certificates.length;i++){
            if (certificates[i].ETHAddress == ETHAddress) {
                for (uint j = i; j < certificates.length - 1; j++) {
    ...
    function deleteProduct(address ETHAddress) public {
