...
contract CropManager {
    ...
    function deleteCrop(address ETHAddress) public {
        for (uint i=0;i<crops.length;i++){
            if (crops[i].ETHAddress == ETHAddress) {
                for (uint j = i; j < crops.length - 1; j++) {
                    crops[j] = crops[j + 1];
                }
                crops.pop();
                break; 
    ...
    function deleteMidterm(address ETHAddress) public {
        for (uint i=0;i<midterm.length;i++){
            if (midterm[i].ETHAddress == ETHAddress) {
                for (uint j = i; j < midterm.length - 1; j++) {
